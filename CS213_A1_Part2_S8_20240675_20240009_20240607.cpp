...
//                                                            Section 8
// Repo Link:https://github.com/youssefah3531/assignment1.git
// Document and Video Link:https://drive.google.com/drive/folders/1Eo73gJrCAXOOExw5psan2b0pOgBWDU4h?usp=drive_link
// ######################################################################################################################################
...
void Blur_Image(string &filepath)
{
    ...
    for (int j = 1; j <= height; j++)
    {
        for (int i = 1; i <= width; i++)
        {
            ...
            integralR[j][i] = vR + integralR[j - 1][i] + integralR[j][i - 1] - integralR[j - 1][i - 1];
            integralG[j][i] = vG + integralG[j - 1][i] + integralG[j][i - 1] - integralG[j - 1][i - 1];
            integralB[j][i] = vB + integralB[j - 1][i] + integralB[j][i - 1] - integralB[j - 1][i - 1];
        }
    }
    for (int j = 0; j < height; j++)
    {
        for (int i = 0; i < width; i++)
        {
            int i0 = max(0, i - half);
            int j0 = max(0, j - half);
            int i1 = min(width - 1, i + half);
            int j1 = min(height - 1, j + half);
            int I0 = i0;
            int J0 = j0;
            int I1 = i1 + 1;
            int J1 = j1 + 1;
            long long totalR = integralR[J1][I1] - integralR[J0][I1] - integralR[J1][I0] + integralR[J0][I0];
            long long totalG = integralG[J1][I1] - integralG[J0][I1] - integralG[J1][I0] + integralG[J0][I0];
            long long totalB = integralB[J1][I1] - integralB[J0][I1] - integralB[J1][I0] + integralB[J0][I0];
            int area = (i1 - i0 + 1) * (j1 - j0 + 1);
            image2(i, j, 0) = (totalR / area);
            image2(i, j, 1) = (totalG / area);
            image2(i, j, 2) = (totalB / area);
        }
    }
    cout << "Do you want to save the photo?\n1-YES\n2-NO" << endl;
    int num;
